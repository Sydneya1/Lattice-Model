...
Num_of_gen = 500  # number of iterations in the simulation
...
def birth(group_size, numcancer, i, j):  # make sure this works and the number of cancer cells is compatible
    ...
    if numcancer != 0:
        y = random.binomial(numcancer, probc, size=None)  
        Grid[i][j] += y
    else:
        y = 0

    x = random.binomial(numcoop, bpfc, size=None)  
    Grid[i][j] += x
    ...
    # mutate
    num_of_mutations = random.binomial(true_new_coop, probm, size=None)  
